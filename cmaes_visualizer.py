...
Optional arguments:
  ...
  --function {quadratic, ellipse,himmelblau, rosenbrock, six-hump-camel}
  --seed SEED
  --frames FRAMES
  --interval INTERVAL
...
Example:
  python cmaes_visualizer.py --function ellipse

  python tools/cmaes_visualizer.py --function himmelblau \
...
parser.add_argument(
    "--function",
    choices=["quadratic", "ellipse", "himmelblau", "rosenbrock", "six-hump-camel"],
)
parser.add_argument(
    "--seed",
    type=int,
...
def ellipse(x1, x2):
    return (x1**2 + (1000*x2)**2)

def ellipse_contour(x1,x2):
    return np.log(ellipse(x1,x2) + 1)
...
elif args.function == "ellipse":
    function_name = "Ellipse function"
    objective = ellipse
    contour_function = ellipse_contour
    global_minimums = [
        (0,0, 0,0),
    ]
    # input domain
    x1_lower_bound, x1_upper_bound = -1.5, 1.5
    x2_lower_bound, x2_upper_bound = -1.5, 1.5
...
optimizer = CMA(mean=np.ones(2), sigma=sigma0, bounds=bounds, seed=seed)
solutions = []
trial_number = 0
...
# use this for animating each iteration, rather than 5000
...
def update(frame):
    ...
    # Take 5000 samples between frames
    n_sample = min(5000, optimizer.population_size - len(solutions))
    for i in range(n_sample):
        x = optimizer.ask()
        evaluation = objective(x[0], x[1])
        solution = (x, evaluation)
        solutions.append(solution)
        trial_number += 1

    # Only plot once per frame (after 5000 trials)
    ax1.plot(optimizer._mean[0], optimizer._mean[1], "ro", alpha=0.7)
...
def main():
    ...
    ani.save(rf"C:\Users\dharvenk\Documents\cmaes_ani{args.function}.mp4")
    #plt.show() - do NOT use for ellipse example as computer won't have enough memory 
